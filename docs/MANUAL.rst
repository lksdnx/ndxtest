...
---------------------
The `ndxtest` package seeks to make the backtesting of trading strategies accessible. In its current version (0.0.2)
...
.. warning::
   Ticker symbol name changes can occur as well. Most times, these are not announced by spglobal.
...
This simple strategy generates lots of signals. With a universe of 500 stocks, you will fully invested for most of the time.

.. note::
   Take care not to combine mutually exclusive elementary conditions or you will end up with no signals.
...
- The same applies vice versa. A new long entry signal will not open a new long position if a short position
  in the same symbol already exists.
...
- Cover short positions if the index has risen by 10% or more within the last 10 days.
...
.. note::
   Note that this time we are also using a stoploss of 10%. The stop loss threshold is always relative to the entry price. A trailing stoploss is not implemented yet.
...
| The strategy outperformed the S&P 500 index!
| However, this strategy was a little tailored to perform well during the corona crash.
| Knowing the future, one tends to fine tune strategies to a specific backtesting time period.
...
The strategy did not beat the index even though there was significant survivorship bias at play.
Of 742 symbols that were included in the index during the time period, 184 were not present in the library.
With these results, buying an ETF on the index would definitely have been the better and easier choice.
...
Before concluding this manual/tutorial, here are some more examples of and remarks on what can be done using the
...
``lambda x: x.close > prev(x.close)`` for example can be used in many ways using other operators and other datapoints such as ``x.low`` or ``x.high``
...
.. note::
   You can enter arbitrary positive numbers, however, signals complete with day 0 and positions are initiated at day 1.
   If anything, only conditions concerning the open of the day of position initiation (1) make sense.
   Anything beyond that would require knowledge of the future.
...
| please give https://github.com/lksdnx/ndxtest a star.
